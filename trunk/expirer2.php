...
$_POST['expiry_second']=0;
	...
	/** modified by webmask */
	$query_published  = mysql_query("SELECT *  FROM `wp_posts` WHERE `ID` = $pid AND post_status='publish'");
	
	if(mysql_num_rows($query_published) <= 0) { return "";}
	/** modified by webmask */
		
	 $query="INSERT INTO wp_page_expiry_info (expiry_time,redirection_url,post_id,method,timestamp) VALUES ($time,'$url',$pid,$method,$timestamp)";
